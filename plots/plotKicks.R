			...
			rng <- c(0,5)
				...
				rng <- c(0,20)
					...
					rng <- c(0,10)
						...
						rng <- c(0,1)
					}else{if(yaxis == "success") {# || yaxis == "majgroup"){
							rng <- c(0,1)
...
plotOnNodes <- function(prefix, nodes_list=seq(11,47,12), link_list=seq(0.2, 0.8, 0.2), edge_list=seq(5, 20, 5), range_list=c(1), xlabel="Link probability",
		netType="erdos-renyi", pbound='true', T_MAX=50, xpar='range', boxptime=FALSE, driftranges=c(0.5), driftbases=c(0.1), updatemodels=c("conf-kick"),
		colours=rainbow(10), yrange=c(0,1), legNodes=FALSE, yaxis="effectivity", env='none', quorum=1) {
	
	pntTypes = c(1,4,5,3,8,7,0,2,6,9,10,11,12,13)
	
	combolength <- length(driftranges)*length(driftbases)*length(link_list)*length(edge_list)*length(range_list)*length(updatemodels)
					...
					for (update in updatemodels){
						for (driftbase in driftbases){
							for (driftrange in driftranges){
								...
								success <- nrow(data[ data$pos >= (nodes*quorum), ]) / nrow(data)
								times <- data[ data$pos == nodes | data$neg == nodes, 'iter']
								dataPlot <- rbind(dataPlot, c(netPar, m, which(driftrange == driftranges)[1], nodes, effectivity,
												success, mean(times), mean(majGroup)/nodes, mean(data$pos)/nodes, mean(data$neg)/nodes, majpos,
												1.96*sqrt( effectivity * (1-effectivity) / nrow(data) ),
												1.96*sqrt( success * (1-success) / nrow(data) ),
												1.96*sd(times)/sqrt(length(times)),
								...
								if (xparv == nodes_list[1]) {
									#ltxt <- paste(update," ",substr(xpar,1,1),":",legv," n:",nodes,sep="")
									ltxt <- paste("u:", update,"b:", driftbase," r:",driftrange,sep="")
									#ltxt <- gsub("rand", "rule", ltxt)
									legTxt <- c(legTxt, ltxt)
									if (legNodes){
										if (m %% 2 == 0){
											collist <- append(collist, 'white' )
											collistB <- append(collistB, colours[m/length(driftranges)] )
										} else {
											collist <- append(collist, colours[(m+1)/length(driftranges)])
											collistB <- append(collistB, 'black')
										}
									}
								}
	...
	#print(dataPlot)
	colnames(dataPlot) <- c("netPar", "loop", "driftrange", "nodes", "effectivity", "success", "time", "majgroup", "pos", "neg", "majpos", 
			"effectivity-stddev", "success-stddev", "time-stddev", "majgroup-stddev", "emin", "epl", "smin", "spl", "tmin", "tpl" )
	#print(colnames(dataPlot))
	if (yaxis != 'time' || !boxptime){
		for (m in seq(1,length(legTxt))){
			if (legNodes){
				countLines <- (((m-1) %% (length(driftranges)*length(driftbases)))+1)
				countPoints  <- (1+(m-countLines)/(length(driftranges)*length(driftbases)))
				points( dataPlot[dataPlot[,2]==m,'nodes'], dataPlot[dataPlot[,2]==m, yaxis], pch=pntTypes[countPoints], col=colours[countPoints], cex=1, lwd=2, type='b', lty=countLines )
			} else {
	...
	if (legNodes){
		...
		if (yaxis == 'time' && boxptime){
			legend(legpos, paste("d-range:", driftranges), pch=22, col=c('black','gray'), pt.bg=c('gray','white'), lty=-1, cex=1, lwd=2, bg='white' )
		} else {
			lgndtxt <- c()
			for (driftrange in driftranges){
				for (dbase in driftbases){
				#for (update in updatemodels){
					lgndtxt <- append(lgndtxt, paste("b:",dbase," r:",driftrange,sep="") )
				}
			}
...
#		txtL <- paste("d-Base: ", driftbases, sep="")
		...
		legend(legpos2, txtL, pch=pntTypes[1:(combolength/length(driftranges))], col=colours[1:(combolength/length(driftranges))], lty=-1, cex=1, lwd=2, bg='white' )
	} else {
		legend(legpos, legTxt, pch=pntTypes[1:combolength], col=colours[1:combolength], lty=0, cex=1, lwd=2, bg='white' )
	}
	return(dataPlot)
